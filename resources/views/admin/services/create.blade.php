...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.services.store') }}" class="space-y-5" enctype="multipart/form-data">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4" required class="w-full rounded-xl border border-slate-200 px-4 py-3">{{ old('description') }}</textarea>
        </div>
        <div>
        ...
        <div>
            ...
            <input id="sort_order" name="sort_order" type="number" class="w-full rounded-xl border border-slate-200 px-4 py-3" value="{{ old('sort_order', 0) }}">
        </div>
